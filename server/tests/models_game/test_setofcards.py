...
def test_setofcards_initialize_multiple_sets(card_2t, two_cards, card_2h):
    cardset1 = SetOfCards(cards=[card_2t])
    cardset2 = SetOfCards(cards=two_cards)
...
def test_setofcards_in_true(two_cards, two_other_cards):
    cardset1 = SetOfCards(cards=two_cards + two_other_cards)
    cardset2 = SetOfCards(cards=two_cards)
    assert (cardset2 in cardset1) is True


def test_setofcards_in_false(two_cards, two_other_cards):
    cardset1 = SetOfCards(cards=two_cards)
    cardset2 = SetOfCards(cards=two_other_cards)
...
def test_setofcards_isdisjoint(card_2h, card_2t):
    cardset1 = SetOfCards(cards=[card_2h])
    cardset2 = SetOfCards(cards=[card_2t])
